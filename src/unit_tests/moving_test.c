...
START_TEST(test_moving) {
  ...
  ck_assert_int_ge(game->field[2][3], FigureSym);
  ck_assert_int_ge(game->field[2][4], FigureSym);
  ck_assert_int_ge(game->field[2][5], FigureSym);
  //////////////////////////////////////////////////////
  ...
  ck_assert_int_ge(game->field[3][7], FigureSym);
  //////////////////////////////////////////////////////
  ...
  ck_assert_int_ge(game->field[2][9], FigureSym);
  ck_assert_int_ge(game->field[3][8], FigureSym);
  ck_assert_int_ge(game->field[3][9], FigureSym);
  ck_assert_int_ge(game->field[3][10], FigureSym);
  //////////////////////////////////////////////////////

  for (int i = 0; i < 4; i++)
    game->next[1][i]++;

  if (CheckingFreePosition() != 4)
    ReturnFigureBack(1, 1);
...
Suite *moving(void) {
  Suite *suite = suite_create("moving");
  TCase *test_case = tcase_create("case_moving");

  tcase_add_test(test_case, test_moving);
